...
interface IProps {
    ...
    date: Date,
    ddlValue?: { key: string | number | undefined,text: string | number | undefined }
  }
 

const ExecDecisionRecord: React.FC<IProps> = ({contextProp, date,ddlValue } : IProps) => {

    const [count, setCount] = React.useState([1]);
    ddlValue = { key: '', text: 'Select an Option'};
...
<td className={styles.tdFileLocation}>DECISION</td>
  ...
  <td className={styles.tdBorder}><DateTimePicker  
    ...
    resolveDelay={1000} /></div></td>
        ...
        selectedKey={ddlValue.key ? '' : ddlValue.key} /></td>
  </tr>  
